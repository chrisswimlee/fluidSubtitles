...
fluidSubtitles is a local Theater caption app for Korean, English, and Thai. Speech recognition and the macOS shell come from [FluidVoice](https://github.com/altic-dev/FluidVoice). This branch adds clause-level Apple Translation, a measured latency clock, and a bounded 3-hour caption budget.
...
1. Direct Core Audio captures one input stream. Packets carry `inputHostTime`.
...
6. English, Thai, and Korean wait for a strong ending plus a language confirm (1.0 / 1.5 / 2.0 s). Parakeet end-of-utterance does not dump the open tail.
...
The live Apple path still has no prompt. On commit, Theater sends the last 1–2 source clauses plus the new one, then peels the new caption. Pronoun and zero-subject Korean can still drift. Do not describe this as a streaming context window on the Neural Engine.
...
Voice engines (Parakeet, Nemotron, Cohere, Whisper) download at first use. Apple Translation downloads a language pack the first time a pair is used. Listen is gated until the pack reports `.installed`. Theater also refuses English-only Parakeet when I speak is Korean or Thai and falls back to an installed engine that can hear that language. No API key is required for the live Theater path.

## Thermal

`thermalState` is shown on the HUD. On serious or critical thermal, Theater skips extra ASR ticks after the 400 ms silence hold. First words and voiced speech still run. Translation is not paused.
