...
final class AIEnhancementSettingsViewModel: ObservableObject {
    ...
    private func probeKeychainAccess() -> KeychainAccessCheckResult {
        let candidates = FluidProduct.keychainLookupIdentities
        ...
        case errSecItemNotFound:
            let addQuery: [String: Any] = [
                kSecClass as String: kSecClassGenericPassword,
                kSecAttrService as String: FluidProduct.keychainService,
                kSecAttrAccount as String: FluidProduct.keychainAccount,
                kSecValueData as String: (try? JSONEncoder().encode([String: String]())) ?? Data(),
            ]
    ...
    private func keychainPermissionExplanation(for status: OSStatus) -> String {
        var message = "\(FluidProduct.displayName) stores provider API keys securely in your macOS Keychain but does not currently have permission to access it."
        if let detail = SecCopyErrorMessageString(status, nil) as String? {
            message += "\n\nmacOS reported: \(detail) (\(status))"
        }
        message += "\n\nClick \"Always Allow\" when the Keychain prompt appears, or open Keychain Access > login > Passwords, locate the \(FluidProduct.displayName) entry, and grant access."
        return message
    }

    private func keychainPersistenceExplanation(for error: Error) -> String {
        var message = "\(FluidProduct.displayName) could not save the API key to your macOS Keychain, so this provider was not verified."
    ...
    func presentKeychainAccessAlert(message: String) {
        let msg = message.isEmpty
            ? "\(FluidProduct.displayName) stores provider API keys securely in your macOS Keychain. Please grant access by choosing \"Always Allow\" when prompted."
            : message
    ...
    func addCurrentAppPromptBinding(for mode: SettingsStore.PromptMode) {
        guard let target = self.resolveBindingTargetApp() else {
            self.appPromptBindingErrorMessage = "Could not detect a target app. Focus another app window (outside \(FluidProduct.displayName)) and try again."
            DebugLogger.shared.info(
