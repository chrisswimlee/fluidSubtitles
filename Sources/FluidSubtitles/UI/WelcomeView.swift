                ...
                VStack(alignment: .leading, spacing: 16) {
                    HStack(spacing: 10) {
                        ...
                        VStack(alignment: .leading, spacing: 2) {
                            Text((self.asr.isAsrReady || self.asr.modelsExistOnDisk) ? "Getting Started" : "Welcome to \(FluidProduct.displayName)")
                                .font(self.theme.typography.title)
                            Text(FluidProduct.tagline + " " + FluidProduct.manifesto)
                            ...
                            VStack(alignment: .leading, spacing: 8) {
                                ...
                                SetupStepView(
                                    ...
                                    description: self.asr.micStatus == .authorized
                                        ? "\(FluidProduct.displayName) has access to your microphone"
                                        : "Allow \(FluidProduct.displayName) to access your microphone for voice input",
                                ...
                                SetupStepView(
                                    step: 3,
                                    title: self.accessibilityEnabled ? "Accessibility Access Enabled" : "Enable Accessibility for Transcription",
                                    description: self.accessibilityEnabled
                                        ? "\(FluidProduct.displayName) can type a translation into other apps. Theater captions do not need this."
                                        : "Needed only if you want to type a translation into another app. Theater captions work without it.",
                                ...
                                SetupStepView(
                                    step: 4,
                                    title: "Open Theater",
                                    description: "Theater is a window you turn on. Listening fills it with the translation.",
                                    status: .completed,
                                    action: {
                                        self.selectedSidebarItem = .liveTranslation
                                    },
                                    actionButtonTitle: "Open Translate"
                                )

                                SetupStepView(
                                    step: 5,
                                    title: self.playgroundUsed ? "You opened Theater" : "Open Theater",
                                    description: self.playgroundUsed
                                        ? "You opened Theater from Translate"
                                        : "Open Theater, then press the mic on that window.",
                                ...
                                Label {
                                    VStack(alignment: .leading, spacing: 2) {
                                        Text("Test Playground")
                                            .font(self.theme.typography.sectionTitle)
                                        Text("Optional dictation practice. Prefer Translate to open Theater and listen for captions.")
                                ...
                                HStack {
                                    ...
                                    Button {
                                        ...
                                        HStack(spacing: 8) {
                                            ...
                                            Text(self.asr.isRunning ? "Stop listening" : "Start listening")
                                        }
                                        .frame(maxWidth: 220)
                                    }
                                    ...
                                    .overlay(
                                        VStack(spacing: 8) {
                                            if self.asr.isRunning {
                                                ...
                                                Text("Press Start listening, then talk")
                                                    .font(self.theme.typography.caption)
                                                    .foregroundStyle(.secondary)
                                            }
                                        }
...
struct OnboardingFlowView: View {
    ...
    @ObservedObject private var settings = SettingsStore.shared
    @ObservedObject private var translationController = LiveTranslationController.shared
    ...
    private enum Step: Int, CaseIterable {
        ...
        var title: String {
            ...
            case .playground:
                return "Try \(FluidProduct.displayName)"
            }
        }
        ...
        var subtitle: String {
            switch self {
            case .landing:
                return "Theater captions on your screen, or Transcription typed into an app."
            case .language:
            ...
            case .permissions:
                return "Microphone is required. Accessibility is only if you want Transcription typed into other apps."
            case .aiEnhancement:
                return "Optional. Translation does not use AI Enhancement."
            case .playground:
                return "Open Theater, press the mic, and say a sentence."
            }
        }
    }
    ...
    private var onboardingTargetLanguages: [TranslationLanguage] {
        let source = TranslationLanguageCatalog.language(id: self.selectedLanguageID)
            ?? TranslationLanguageCatalog.english
        return TranslationLanguageCatalog.targets(excluding: source)
    }
            ...
            ZStack {
                VStack(alignment: .center, spacing: self.theme.metrics.onboardingSurface.landing.sectionSpacing) {
                    FluidOnboardingLandingHero(
                        ...
                        accentTitle: "one caption at a time.",
                        firstDetail: FluidProduct.manifesto,
                        secondDetail: "Theater captions on screen, or typed translation into the app you clicked."
                    ) {
    ...
    private func selectOnboardingLanguage(_ language: VoiceEngineLanguage) {
        ...
        withTransaction(transaction) {
            ...
            if let translationLanguage = TranslationLanguageCatalog.language(id: language.id) {
                ...
                if self.settings.translationTargetLanguageID == translationLanguage.id {
                    self.settings.translationTargetLanguageID = TranslationLanguageCatalog.defaultTarget(forSource: translationLanguage).id
                }
            }
                ...
                VStack(spacing: 0) {
                    ...
                    ScrollView(.vertical, showsIndicators: false) {
                        VStack(spacing: 0) {
                            ...
                            Text("Microphone is required. Accessibility is only if you want Transcription typed into another app.")
                                .font(.system(size: 15, weight: .medium))
                                .foregroundStyle(Color.white.opacity(0.62))
                                .padding(.bottom, 28)
                            ...
                            VStack(spacing: 14) {
                                ...
                                if !self.isAccessibilityReady {
                                    Text("Already enabled it? \(FluidProduct.displayName) will update when macOS confirms access.")
                                        .font(.system(size: 12, weight: .medium))
                ...
                VStack(spacing: 0) {
                    ...
                    ScrollView(.vertical, showsIndicators: false) {
                        VStack(spacing: 0) {
                            ...
                            Text("Try a translated sentence.")
                                .font(.system(size: 28, weight: .semibold))
                                .foregroundStyle(.white)
                                .multilineTextAlignment(.center)
                                .lineLimit(2)
                            ...
                            Text("Open Theater, then press the mic on that window. The translation appears there.")
                                .font(.system(size: 15, weight: .medium))
                                .foregroundStyle(Color.white.opacity(0.62))
                                .multilineTextAlignment(.center)
                                .padding(.bottom, 22)
                    ...
                    self.cinematicFooter(
                        ...
                        skipAction: {
                            AnalyticsService.shared.skipOnboardingTryout(origin: self.settings.analyticsOnboardingOrigin)
                            self.settings.onboardingPlaygroundSkipped = true
                            self.finishSetupFromPlayground(outcome: .skipped)
                        }
                    )
                }
                .frame(width: proxy.size.width, height: proxy.size.height)
    ...
    private func handlePrimaryAction() {
        ...
        if self.step == .playground {
            ...
            self.finishSetupFromPlayground(outcome: .completed)
            return
        }
    ...
    private func finishSetupFromPlayground(outcome: AnalyticsOnboardingOutcome) {
        let origin = self.settings.analyticsOnboardingOrigin
        self.markAISkipped()
        self.completeCurrentStep(
            outcome: outcome,
