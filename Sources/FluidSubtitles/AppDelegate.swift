...
class AppDelegate: NSObject, NSApplicationDelegate, UNUserNotificationCenterDelegate {
    ...
    private func checkForUpdatesAutomatically() {
        ...
        Task {
            ...
            guard let repository = FluidProduct.updateRepository else {
                DebugLogger.shared.debug("Automatic update check skipped (no published repository)", source: "AppDelegate")
                return
            }

            DebugLogger.shared.info("Performing automatic update check for \(repository.owner)/\(repository.repo)", source: "AppDelegate")
    ...
    private func showUpdateNotification(version: String) {
        ...
        alert.informativeText = "\(FluidProduct.displayName) \(version) is now available. Would you like to install it now?\n\nThe app will restart automatically after installation."
