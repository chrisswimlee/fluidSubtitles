...
nonisolated enum StreamingPreviewMode: Equatable, Sendable {
...
protocol TranscriptionProvider {
    ...
    /// How live previews should copy PCM from the retained ring.
    var streamingPreviewMode: StreamingPreviewMode { get }

    /// Advance an incremental session with only newly captured samples.
    func transcribeStreamingDelta(
        _ newSamples: [Float],
        totalSampleCount: Int
    ) async throws -> ASRTranscriptionResult

    /// Finalize an incremental session with only the unaccepted tail.
...
extension TranscriptionProvider {
    ...
    var streamingPreviewMode: StreamingPreviewMode { .trailingWindow }

    func transcribeStreamingDelta(
        _ newSamples: [Float],
        totalSampleCount _: Int
    ) async throws -> ASRTranscriptionResult {
        try await self.transcribeStreaming(newSamples)
    }
    ...
    func transcribeFinalDelta(
        ...
        try await self.transcribeFinal(newSamples)
    }
