...
final class NemotronProvider: TranscriptionProvider {
    ...
    func transcribeStreamingDelta(
        ...
        guard self.mode != .offline else {
            return try await self.transcribeFinal(newSamples)
        }
        guard let manager = self.manager else {
            throw Self.makeError("Nemotron provider is not ready.")
        }
        ...
        if !newSamples.isEmpty {
            do {
                try await manager.appendAudio(Self.makeBuffer(newSamples))
                try await manager.processBufferedAudio()
            } catch {
                await self.stopComponentProfilingIfNeeded(on: manager)
                throw error
    ...
    func transcribeFinalDelta(
        ...
        await self.finishComponentProfilingIfNeeded(on: manager, samples: newSamples)
        await manager.reset()
        self.streamedSampleCount = 0
        return ASRTranscriptionResult(text: text, confidence: text.isEmpty ? 0 : 1)
    }
    ...
    func releaseMemory() async {
        ...
        self.streamedSampleCount = 0
        self.activeLanguageCode = nil
        DebugLogger.shared.info("Nemotron: released in-memory models", source: "NemotronProvider")
    }
