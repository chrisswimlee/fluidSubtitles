...
private final nonisolated class HotkeyState: @unchecked Sendable {
    ...
    var isTranslateInsertKeyPressed = false
...
final class GlobalHotkeyManager: NSObject {
    ...
    private nonisolated var isTranslateInsertKeyPressed: Bool {
        get { self.state.withLock { self.state.isTranslateInsertKeyPressed } }
        set { self.state.withLock { self.state.isTranslateInsertKeyPressed = newValue } }
    }
    ...
    init(
        ...
        self.primaryShortcuts = Self.effectivePrimaryShortcuts(primaryShortcuts)
    ...
    func updatePrimaryShortcuts(_ newShortcuts: [HotkeyShortcut]) {
        self.primaryShortcuts = Self.effectivePrimaryShortcuts(newShortcuts)
        DebugLogger.shared.info(
            "Updated listening hotkeys enabled=\(SettingsStore.shared.listeningHotkeyEnabled)",
    ...
    private static func effectivePrimaryShortcuts(_ shortcuts: [HotkeyShortcut]) -> [HotkeyShortcut] {
        SettingsStore.shared.listeningHotkeyEnabled ? shortcuts : []
    }
    ...
    func setTranslateInsertCallback(_ callback: @escaping () async -> Void) {
        self.translateInsertCallback = callback
    }

    func setCaptionListenCallback(_ callback: @escaping () -> Void) {
    ...
    private func handleKeyEvent(proxy: CGEventTapProxy, type: CGEventType, event: CGEvent) -> Unmanaged<CGEvent>? {
        ...
        case .keyDown:
            ...
            if self.handleTranslateInsertKeyDown(
                keyCode: keyCode,
            ...
            if self.handleCaptionListenKeyDown(
                keyCode: keyCode,
                modifiers: eventModifiers,
                isAutorepeat: event.getIntegerValueField(.keyboardEventAutorepeat) != 0
            ) {
        ...
        case .keyUp:
            ...
            if self.handleTranslateInsertKeyUp(keyCode: keyCode) { return nil }
        ...
        case .flagsChanged:
            ...
            if self.handleTranslateInsertFlagsChanged(keyCode: keyCode, modifiers: eventModifiers) { return nil }
            if self.handleCaptionListenFlagsChanged(keyCode: keyCode, modifiers: eventModifiers) { return nil }
    ...
    func resetModifierOnlyShortcutTracking(reason: ModifierTrackingResetReason = .shortcutCapture) {
        let shouldStopActiveHold = self.hotkeyMode != .toggle
            ...
            && (self.isKeyPressed || self.isPromptModeKeyPressed || self.isPromptAssignmentKeyPressed || self.isTranslateInsertKeyPressed)
        ...
        self.isPromptModeKeyPressed = false
        self.isPromptAssignmentKeyPressed = false
        self.isTranslateInsertKeyPressed = false
    ...
    private func isTranslateInsertSessionActive() -> Bool {
        LiveTranslationController.shared.listenKind == .insert && self.asrService.isRunning
    }

    private func handleTranslateInsertKeyDown(
        keyCode: UInt16,
        modifiers: NSEvent.ModifierFlags,
        isAutorepeat: Bool
    ) -> Bool {
        guard SettingsStore.shared.translationInsertHotkeyEnabled,
              let shortcut = SettingsStore.shared.translationInsertHotkeyShortcut,
              !shortcut.isModifierOnlyShortcut,
              shortcut.matches(keyCode: keyCode, modifiers: modifiers)
        else { return false }
        guard !isAutorepeat else { return true }

        switch self.hotkeyMode {
        case .hold:
            if !self.isTranslateInsertKeyPressed {
                self.cancelPendingReleaseStop(for: .translateInsert)
                self.clearHoldModeStartTriggered(for: .translateInsert)
                self.isTranslateInsertKeyPressed = true
                if self.asrService.isRunningOrStarting {
                    if self.isTranslateInsertSessionActive() {
                        self.stopRecordingIfNeeded()
                    }
                } else {
                    self.triggerTranslateInsert()
                }
                self.markHoldModeStartTriggered(for: .translateInsert)
            }
        case .automatic:
            if !self.isTranslateInsertKeyPressed {
                self.isTranslateInsertKeyPressed = true
                let isSameMode = self.isTranslateInsertSessionActive()
                self.beginAutomaticPress(for: .translateInsert, wasTargetActive: isSameMode)
                if self.asrService.isRunning {
                    if isSameMode {
                        // Wait for release to stop.
                    } else {
                        return false
                    }
                } else {
                    self.triggerTranslateInsert()
                    self.markAutomaticPressStarted(for: .translateInsert)
                }
            }
        case .toggle:
            if self.asrService.isRunningOrStarting {
    ...
    private func handleTranslateInsertKeyUp(keyCode: UInt16) -> Bool {
        guard self.isTranslateInsertKeyPressed,
              let shortcut = SettingsStore.shared.translationInsertHotkeyShortcut,
              keyCode == shortcut.keyCode
        else { return false }
        ...
        case .hold:
            self.isTranslateInsertKeyPressed = false
            _ = self.finishHoldModeStartTriggered(for: .translateInsert)
            if self.isTranslateInsertSessionActive() {
                self.stopRecordingAfterRelease(for: .translateInsert, label: "Translate insert")
            }
        case .automatic:
            self.isTranslateInsertKeyPressed = false
            self.handleAutomaticKeyRelease(for: .translateInsert, label: "Translate insert")
        case .toggle:
            break
        }
        return true
    ...
    private func handleTranslateInsertFlagsChanged(
        keyCode: UInt16,
        modifiers: NSEvent.ModifierFlags
    ) -> Bool {
        guard SettingsStore.shared.translationInsertHotkeyEnabled,
        ...
        return self.handleModifierOnlyShortcutFlagsChanged(
            behavior: .init(
                shortcut: shortcut,
                isEnabled: true,
                holdModeType: .translateInsert,
                holdStartMessage: "Translate insert modifier held (hold mode) - starting",
                holdReleaseMessage: "Translate insert modifier released (hold mode) - stopping",
                toggleIgnoredMessage: "Translate insert modifier released but another key was pressed - ignoring",
                isModeKeyPressed: { self.isTranslateInsertKeyPressed },
                setModeKeyPressed: { self.isTranslateInsertKeyPressed = $0 },
                onHoldStart: { self.triggerTranslateInsert() },
                onToggleRelease: {
                    if self.asrService.isRunningOrStarting {
                        if self.isTranslateInsertSessionActive() {
                            self.stopRecordingIfNeeded()
                ...
                isTargetModeActive: { self.isTranslateInsertSessionActive() }
            ),
            keyCode: keyCode,
            modifiers: modifiers
        )
    ...
    private func triggerTranslateInsert() {
        Task { @MainActor [weak self] in
            guard let self else { return }
            guard self.canTriggerRecordingAction("Translate insert hotkey") else { return }
            DebugLogger.shared.info("Translate insert hotkey triggered", source: "GlobalHotkeyManager")
            await self.translateInsertCallback?()
        }
    }

    private func handleCaptionListenKeyDown(
        keyCode: UInt16,
        modifiers: NSEvent.ModifierFlags,
        isAutorepeat: Bool
    ) -> Bool {
        guard SettingsStore.shared.captionListenHotkeyEnabled,
              let shortcut = SettingsStore.shared.captionListenHotkeyShortcut,
              !shortcut.isModifierOnlyShortcut,
              shortcut.matches(keyCode: keyCode, modifiers: modifiers)
        else { return false }
        guard !isAutorepeat else { return true }
    ...
    private func handleCaptionListenFlagsChanged(
        keyCode: UInt16,
        modifiers: NSEvent.ModifierFlags
    ) -> Bool {
        guard SettingsStore.shared.captionListenHotkeyEnabled,
        ...
        let decision = ModifierOnlyShortcutFlagsDecision.evaluate(
            ...
            modifiers: modifiers,
            state: ModifierOnlyShortcutTrackingState(
                pressedModifierKeyCodes: self.synchronizedPressedModifierKeyCodes(
                    changedKeyCode: keyCode,
                    modifiers: modifiers
                ),
                activeModifierOnlyType: nil,
    ...
    private func triggerCaptionListen() {
        Task { @MainActor [weak self] in
            guard let self else { return }
            guard self.canTriggerRecordingAction("Theater Listen hotkey") else { return }
            DebugLogger.shared.info("Theater Listen hotkey triggered", source: "GlobalHotkeyManager")
            self.captionListenCallback?()
        }
    }
