...
final class FluidAudioProvider: TranscriptionProvider {
    ...
    var acceptedIncrementalSampleCount: Int {
        self.incrementalAcceptedSampleCount
    }
    ...
    func transcribeFinalDelta(
        ...
        do {
            let result = try await session.finish(finalAudioSamples: newSamples)
            let text = result.text.trimmingCharacters(in: .whitespacesAndNewlines)
            if text.isEmpty, !self.latestStreamingPreviewText.isEmpty {
                return ASRTranscriptionResult(text: self.latestStreamingPreviewText, confidence: result.confidence)
            }
            return ASRTranscriptionResult(text: result.text, confidence: result.confidence)
        } catch {
            if !self.latestStreamingPreviewText.isEmpty {
                DebugLogger.shared.warning(
                    "FluidAudioProvider: Incremental finish failed; using last preview text",
                    source: "FluidAudioProvider"
                )
                return ASRTranscriptionResult(text: self.latestStreamingPreviewText, confidence: 1)
            }
            throw error
        }
    }
    ...
    private func transcribeIncrementalFinal(_ samples: [Float]) async throws -> ASRTranscriptionResult {
        guard self.incrementalSession != nil else {
            throw NSError(
                domain: "FluidAudioProvider",
                code: -3,
                userInfo: [NSLocalizedDescriptionKey: "Incremental ASR session cannot finalize this recording"]
        ...
        let tail: [Float]
        if samples.count >= acceptedBeforeFinal, acceptedBeforeFinal > 0 {
            tail = Array(samples[acceptedBeforeFinal...])
        } else {
            tail = samples
        }
        return try await self.transcribeFinalDelta(tail, totalSampleCount: max(acceptedBeforeFinal + tail.count, samples.count))
    }
