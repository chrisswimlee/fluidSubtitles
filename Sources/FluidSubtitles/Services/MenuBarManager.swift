...
final class MenuBarManager: NSObject, ObservableObject, NSMenuDelegate {
    ...
    private func buildMenuStructure() {
        ...
        // Open Main Window
        let openItem = NSMenuItem(title: "Open \(FluidProduct.displayName)", action: #selector(openMainWindow), keyEquivalent: "")
        ...
        let quitItem = NSMenuItem(
            title: "Quit \(FluidProduct.displayName)",
    ...
    @objc private func rollbackToPreviousVersion(_ sender: Any?) {
        ...
        confirm.informativeText = "This will restore the backup and relaunch \(FluidProduct.displayName)."
        ...
        Task { @MainActor in
            do {
                ...
                success.informativeText = "Rolled back to \(availableVersion). \(FluidProduct.displayName) will relaunch shortly."
    ...
    @objc private func openLiveTranslation() {
        self.openNavigationDestination(.liveTranslation)
    }
