...
final class ASRService: ObservableObject {
    ...
    private var lastCompletedAudioFile: DictationAudioMetadata?
    private var streamingWavWriter: DictationAudioHistoryStore.StreamingWavWriter?
    private var committedStreamingText = ""
    private var idleMemoryReleaseTask: Task<Void, Never>?
    private static let idleMemoryReleaseNanoseconds: UInt64 = 5 * 60 * 1_000_000_000
    ...
    private func beginStreamingWavWriterIfNeeded() {
    ...
    private func finishStreamingWavWriter(model: String) {
    ...
    private func abortStreamingWavWriter() {
    ...
    private func dropRetainedAudioAfterPreview(currentSampleCount: Int, usedIncrementalDelta: Bool) {
    ...
    private func cancelIdleMemoryRelease() {
        self.idleMemoryReleaseTask?.cancel()
        self.idleMemoryReleaseTask = nil
    }

    private func scheduleIdleMemoryRelease() {
    ...
    private func releaseIdleModelMemoryIfNeeded() async {
        guard self.isRunning == false,
...
private final nonisolated class AudioCapturePipeline: @unchecked Sendable {
    private let audioBuffer: ThreadSafeAudioBuffer
    private let onAcceptedSamples: ([Float]) -> Void
