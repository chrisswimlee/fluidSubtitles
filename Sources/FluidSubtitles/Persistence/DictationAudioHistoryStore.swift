...
final nonisolated class DictationAudioHistoryStore: @unchecked Sendable {
    ...
    func beginStreamingWrite(entryID: UUID, timestamp: Date) throws -> StreamingWavWriter {
        let directory = try self.audioDirectory()
        let fileName = self.pendingSaveLock.withLock { () -> String in
            let fileName = self.audioFileName(entryID: entryID, timestamp: timestamp)
            self.pendingSaveFileNames.insert(fileName)
        ...
        let url = directory.appendingPathComponent(fileName, isDirectory: false)
        do {
            return try StreamingWavWriter(url: url, fileName: fileName)
        } catch {
            self.completePendingSave(fileName: fileName)
            throw error
        }
    ...
    func suggestedAudioExportFilename(for date: Date = Date()) -> String {
        "fluidSubtitles_Audio_\(Self.fileTimestampFormatter.string(from: date)).zip"
    }

    func suggestedPairExportFilename(for entry: TranscriptionHistoryEntry) -> String {
        "fluidSubtitles_Pair_\(Self.fileTimestampFormatter.string(from: entry.timestamp))_\(entry.id.uuidString.prefix(8)).zip"
    }
    ...
    private func exportArchive(entries: [TranscriptionHistoryEntry], to destinationURL: URL) throws {
        ...
        let staging = self.fileManager.temporaryDirectory
            .appendingPathComponent("fluidsubtitles-audio-\(UUID().uuidString)", isDirectory: true)
    ...
    private func audioDirectory(createIfNeeded: Bool = true) throws -> URL {
        let directory = AppSupportDirectory.url(fileManager: self.fileManager)
    ...
    final class StreamingWavWriter: @unchecked Sendable {
        ...
        init(url: URL, fileName: String) throws {
            self.url = url
            ...
            self.handle = handle
            try handle.write(contentsOf: Self.placeholderHeader(sampleRate: self.sampleRate, channels: self.channels))
        }
        ...
        func append(_ samples: [Float]) {
            ...
            guard !self.aborted, let handle = self.handle else { return }
            var bytes = Data()
            bytes.reserveCapacity(samples.count * 2)
            for sample in samples {
                let clamped = max(-1.0, min(1.0, sample))
                let scaled = Int16((clamped * Float(Int16.max)).rounded())
                DictationAudioHistoryStore.appendInt16LE(scaled, to: &bytes)
            }
            do {
                ...
                DebugLogger.shared.warning(
                    "Streaming WAV append failed: \(error.localizedDescription)",
                    source: "DictationAudioHistoryStore"
                )
            }
        }

        func finish(model: String?) throws -> DictationAudioMetadata {
            self.lock.lock()
            defer { self.lock.unlock() }
            guard !self.aborted else {
                throw DictationAudioHistoryError.audioMissing
            }
            guard let handle = self.handle else {
                throw DictationAudioHistoryError.audioMissing
            }
            let dataByteCount = self.sampleCount * 2
            ...
            try handle.close()
            self.handle = nil
            let durationMilliseconds = self.sampleRate > 0
                ? Int((Double(self.sampleCount) / Double(self.sampleRate) * 1000).rounded())
                : 0
            return DictationAudioMetadata(
        ...
        func abort() {
            ...
            let alreadyAborted = self.aborted
            self.aborted = true
            self.handle = nil
            self.lock.unlock()
            try? handle?.close()
            ...
            DictationAudioHistoryStore.shared.completePendingSave(fileName: self.fileName)
        }

        private static func placeholderHeader(sampleRate: Int, channels: Int) -> Data {
            self.header(sampleRate: sampleRate, channels: channels, dataByteCount: 0)
        }

        private static func header(sampleRate: Int, channels: Int, dataByteCount: Int) -> Data {
            ...
            DictationAudioHistoryStore.appendUInt32LE(UInt32(36 + dataByteCount), to: &data)
            data.append(contentsOf: Array("WAVE".utf8))
            data.append(contentsOf: Array("fmt ".utf8))
            DictationAudioHistoryStore.appendUInt32LE(16, to: &data)
            DictationAudioHistoryStore.appendUInt16LE(1, to: &data)
            DictationAudioHistoryStore.appendUInt16LE(UInt16(channels), to: &data)
            DictationAudioHistoryStore.appendUInt32LE(UInt32(sampleRate), to: &data)
            DictationAudioHistoryStore.appendUInt32LE(UInt32(byteRate), to: &data)
            DictationAudioHistoryStore.appendUInt16LE(UInt16(blockAlign), to: &data)
            DictationAudioHistoryStore.appendUInt16LE(UInt16(bitsPerSample), to: &data)
            data.append(contentsOf: Array("data".utf8))
            DictationAudioHistoryStore.appendUInt32LE(UInt32(dataByteCount), to: &data)
            return data
        }
    }
