...
final class KeychainService {
    ...
    /// Refreshes the process cache after the user returns to the app, so
    ...
    func legacyProviderEntries() throws -> [String: String] {
        ...
        for identity in FluidProduct.keychainLookupIdentities {
            result.merge(try self.legacyProviderEntries(in: identity.service)) { current, _ in current }
        }
        return result
    }

    private func legacyProviderEntries(in service: String) throws -> [String: String] {
        ...
        case errSecSuccess:
            ...
            for attributes in attributesArray {
                guard let providerID = attributes[kSecAttrAccount as String] as? String,
                      !FluidProduct.keychainLookupIdentities.map(\.account).contains(providerID)
                else {
                    continue
                }

                var dataQuery = self.legacyQuery(for: providerID, service: service)
    ...
    func removeLegacyEntries(providerIDs: [String] = []) throws {
        ...
        for providerID in targets {
            for identity in FluidProduct.keychainLookupIdentities {
                ...
                let status = SecItemDelete(self.legacyQuery(for: providerID, service: service) as CFDictionary)
    ...
    private func readStoredKeys() throws -> [String: String] {
        ...
        for identity in FluidProduct.keychainLookupIdentities {
            if let keys = try self.readAggregatedKeys(service: identity.service, account: identity.account) {
                return keys
            }
        }
        return [:]
    ...
    private func readAggregatedKeys(service: String, account: String) throws -> [String: String]? {
        var query = self.aggregatedQuery(service: service, account: account)
    ...
    private func aggregatedQuery(service: String? = nil, account: String? = nil) -> [String: Any] {
        [
            kSecClass as String: kSecClassGenericPassword,
    ...
    private func legacyQuery(for providerID: String, service: String? = nil) -> [String: Any] {
        [
            kSecClass as String: kSecClassGenericPassword,
            kSecAttrService as String: service ?? self.service,
