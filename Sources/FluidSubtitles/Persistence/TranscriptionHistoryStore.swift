...
final class TranscriptionHistoryStore: ObservableObject {
    ...
    static let publishedPageLimit = 200

    @Published private(set) var entries: [TranscriptionHistoryEntry] = []
    @Published private(set) var totalEntryCount = 0
    @Published var selectedEntryID: UUID?
    private var summaryRecords: [HistorySummaryRecord] = []
    private var latestFullEntry: TranscriptionHistoryEntry?
    private var unsavedUpserts: [UUID: TranscriptionHistoryEntry] = [:]
    ...
    var selectedEntry: TranscriptionHistoryEntry? {
        ...
        return self.entries.first(where: { $0.id == id }) ?? self.latestFullEntry
    }

    var latestClipboardText: String? {
        (self.latestFullEntry ?? self.entries.first)?.clipboardText
    }

    var hasHistory: Bool {
        self.totalEntryCount > 0 || !self.entries.isEmpty
    }
    ...
    func addEntry(
        ...
        self.summaryRecords.insert(HistoryTextMetrics.summaryRecord(for: entry), at: 0)
        self.latestFullEntry = entry
        self.totalEntryCount = self.summaryRecords.count
        ...
        DebugLogger.shared.debug("Added transcription to history (total: \(self.totalEntryCount))", source: "TranscriptionHistoryStore")
    }
    ...
    func deleteEntry(id: UUID) {
        ...
        if let audio = self.entries.first(where: { $0.id == id })?.audio
            ?? (self.latestFullEntry?.id == id ? self.latestFullEntry?.audio : nil)
        {
            DictationAudioHistoryStore.shared.deleteAudio(fileName: audio.fileName)
        }
        let previousCount = self.entries.count
        let hadSummary = self.summaryRecords.contains { $0.id == id }
        self.entries.removeAll { $0.id == id }
        self.summaryRecords.removeAll { $0.id == id }
        if self.latestFullEntry?.id == id {
            self.latestFullEntry = self.entries.first
        }
        self.totalEntryCount = self.summaryRecords.count
        if self.entries.count != previousCount || hadSummary {
            self.refreshTodaySummary()
        }
    ...
    func deleteEntries(ids: Set<UUID>) {
        ...
        let previousCount = self.entries.count
        let hadSummary = self.summaryRecords.contains { ids.contains($0.id) }
        self.entries.removeAll { ids.contains($0.id) }
        self.summaryRecords.removeAll { ids.contains($0.id) }
        if let latest = self.latestFullEntry, ids.contains(latest.id) {
            self.latestFullEntry = self.entries.first
        }
        self.totalEntryCount = self.summaryRecords.count
        if self.entries.count != previousCount || hadSummary {
    ...
    func clearAllHistory() {
        ...
        self.summaryRecords.removeAll()
        self.latestFullEntry = nil
    ...
    func applySearchQuery(_ query: String) {
        ...
        self.searchTask = Task { @MainActor [weak self] in
            ...
            do {
                ...
                if trimmed.isEmpty {
                    self.summaryRecords = self.mergePendingSummaries(into: loaded.summaries)
                    self.totalEntryCount = self.summaryRecords.count
                    self.trimPublishedPageIfNeeded()
                }
            } catch {
                DebugLogger.shared.debug("History search failed: \(error.localizedDescription)", source: "TranscriptionHistoryStore")
            }
        }
    }
    ...
    var aiProcessedCount: Int {
        self.summaryRecords.filter(\.wasAIProcessed).count
    }

    func makeBackupPayload() async throws -> [TranscriptionHistoryEntry] {
        try await self.writer.load()
    }
    ...
    func restore(from payload: [TranscriptionHistoryEntry]) {
        ...
        self.summaryRecords = payload.map { HistoryTextMetrics.summaryRecord(for: $0) }
        self.entries = Array(sorted.prefix(Self.publishedPageLimit))
        self.latestFullEntry = self.entries.first
        self.totalEntryCount = payload.count
        self.refreshTodaySummary()
        self.selectedEntryID = self.entries.first?.id
        self.persist(upserts: payload, replacing: true)
    }
    ...
    func attachAudio(
        ...
        guard let index = self.entries.firstIndex(where: { $0.id == entryID }) else {
            if self.latestFullEntry?.id == entryID, let latest = self.latestFullEntry {
                let updated = latest.replacingAudio(audio)
                self.latestFullEntry = updated
                self.persist(upserts: [updated])
                self.scheduleAutomaticAudioPruneToBudget()
                return
            }
            DictationAudioHistoryStore.shared.deleteAudio(fileName: audio.fileName)
        ...
        if self.latestFullEntry?.id == entryID {
            self.latestFullEntry = self.entries[index]
        }
    ...
    func deleteAllSavedAudio() -> Int {
        ...
        let removedCount = changed.count
        self.entries = self.entries.map { $0.replacingAudio(nil) }
        if let latest = self.latestFullEntry {
            self.latestFullEntry = latest.replacingAudio(nil)
        }
        self.persist(upserts: changed)
        Task { @MainActor [weak self] in
            guard let self else { return }
            let remaining = (try? await self.writer.loadEntriesWithAudio()) ?? []
            guard !remaining.isEmpty else { return }
            self.persist(upserts: remaining.map { $0.replacingAudio(nil) })
        }
    ...
    func pruneAudioToBudget() -> Int {
        ...
        return self.pruneAudioToBudget(currentBytes: currentBytes, budgetBytes: budgetBytes, audioEntries: nil)
    }

    private func pruneAudioToBudget(
        currentBytes initialCurrentBytes: Int64,
        budgetBytes: Int64,
        audioEntries: [TranscriptionHistoryEntry]?
    ) -> Int {
        ...
        var candidates = (audioEntries ?? self.entries)
            .filter { $0.audio != nil }
            .sorted { $0.timestamp < $1.timestamp }
        let referencedFileNames = Set(
            (self.entries + candidates).compactMap { $0.audio?.fileName }
        )
        ...
        for index in candidates.indices {
            guard let audio = candidates[index].audio else { continue }
            ...
            candidates[index] = candidates[index].replacingAudio(nil)
            changed.append(candidates[index])
            prunedCount += 1
        ...
        if prunedCount > 0 {
            ...
            self.entries = self.entries.map { changedByID[$0.id] ?? $0 }
            if let latest = self.latestFullEntry, let updated = changedByID[latest.id] {
                self.latestFullEntry = updated
            }
            self.persist(upserts: changed)
    ...
    private func scheduleAutomaticAudioPruneToBudget() {
        ...
        self.automaticAudioBudgetTask = Task { @MainActor [weak self] in
            ...
            while true {
                ...
                let audioEntries = (try? await self.writer.loadEntriesWithAudio()) ?? []
                guard gate.accepts(
    ...
    private func persist(upserts: [TranscriptionHistoryEntry] = [], deletes: [UUID] = [], replacing: Bool = false) {
        ...
        if replacing {
            self.unsavedUpserts = Dictionary(upserts.map { ($0.id, $0) }, uniquingKeysWith: { _, last in last })
        } else {
            for id in deletes {
                self.unsavedUpserts.removeValue(forKey: id)
            }
            for entry in upserts {
        ...
        self.writer.write(upserts: upserts, deletes: deletes, replacing: replacing) { error in
            Task { @MainActor in
                ...
                for entry in upserts {
                    if self.unsavedUpserts[entry.id] == entry {
                        self.unsavedUpserts.removeValue(forKey: entry.id)
                    }
                }
                self.persistenceError = nil
            }
    ...
    func retryPersistence() {
        ...
        let unsaved = Array(self.unsavedUpserts.values)
        self.writer.write(upserts: unsaved, replacing: false) { error in
            Task { @MainActor in
                if let error {
                    self.persistenceError = "History could not be saved. \(error.localizedDescription)"
                    return
                }
    ...
    private func refreshTodaySummary() {
        ...
        self.todaySummaryTask = Task { @MainActor [weak self] in
            ...
            while true {
                ...
                let day = self.summaryCalendar().dateInterval(of: .day, for: self.summaryNow())
                let snapshot = self.summaryRecords
                let summary = await Task.detached(priority: .utility) {
                    Self.calculateTodaySummary(records: snapshot, day: day)
                }.value
    ...
    private nonisolated static func calculateTodaySummary(records: [HistorySummaryRecord], day: DateInterval?) -> TodaySummary {
        guard let day else { return TodaySummary(words: 0, transcriptions: 0) }
        let totals = records.reduce(into: (words: 0, transcriptions: 0)) { result, record in
            guard record.timestamp >= day.start, record.timestamp < day.end else { return }
            result.words += record.words
            result.transcriptions += 1
        }
        return TodaySummary(words: totals.words, transcriptions: totals.transcriptions)
    }

    private func trimPublishedPageIfNeeded() {
        guard self.activeSearchQuery.isEmpty, self.entries.count > Self.publishedPageLimit else { return }
        self.entries = Array(self.entries.prefix(Self.publishedPageLimit))
    }

    private func mergePending(into loaded: [TranscriptionHistoryEntry]) -> [TranscriptionHistoryEntry] {
        if self.pendingReplacement {
            return Array(self.pendingUpserts.values).sorted { $0.timestamp > $1.timestamp }
        }
        var merged = loaded.filter { !self.pendingDeletes.contains($0.id) && self.pendingUpserts[$0.id] == nil }
        merged.append(contentsOf: self.pendingUpserts.values)
        return merged.sorted { $0.timestamp > $1.timestamp }
    }

    private func mergePendingSummaries(into loaded: [HistorySummaryRecord]) -> [HistorySummaryRecord] {
        if self.pendingReplacement {
            return Array(self.pendingUpserts.values).map { HistoryTextMetrics.summaryRecord(for: $0) }
        }
        var merged = loaded.filter { !self.pendingDeletes.contains($0.id) && self.pendingUpserts[$0.id] == nil }
        merged.append(contentsOf: self.pendingUpserts.values.map { HistoryTextMetrics.summaryRecord(for: $0) })
        return merged.sorted { $0.timestamp > $1.timestamp }
    }

    func entriesWithSavedAudio() async throws -> [TranscriptionHistoryEntry] {
        try await self.writer.loadEntriesWithAudio()
    }
}
...
extension TranscriptionHistoryStore {
    ...
    private nonisolated static func countWords(in text: String) -> Int {
        HistoryTextMetrics.wordCount(in: text)
    }

    /// Total words across all transcriptions
    var totalWords: Int {
        self.summaryRecords.reduce(0) { $0 + $1.words }
    }
    ...
    var averageWordsPerTranscription: Int {
        guard self.totalEntryCount > 0 else { return 0 }
        return self.totalWords / self.totalEntryCount
    }
    ...
    private var activeDays: [Date] {
        ...
        for record in self.summaryRecords {
    ...
    func dailyWordCounts(days: Int) -> [(date: Date, words: Int)] {
        ...
        for dayOffset in 0..<days {
            ...
            let dayEntries = self.summaryRecords.filter { calendar.isDate($0.timestamp, inSameDayAs: date) }
            let words = dayEntries.reduce(0) { $0 + $1.words }
    ...
    func dailyTranscriptionCounts(days: Int) -> [(date: Date, count: Int)] {
        ...
        for dayOffset in 0..<days {
            ...
            let count = self.summaryRecords.filter { calendar.isDate($0.timestamp, inSameDayAs: date) }.count
    ...
    var aiEnhancementRate: Int {
        guard self.totalEntryCount > 0 else { return 0 }
        return (self.aiProcessedCount * 100) / self.totalEntryCount
    }
    ...
    var peakHour: Int? {
        guard !self.summaryRecords.isEmpty else { return nil }
    ...
    var longestTranscriptionWords: Int {
        self.summaryRecords.map(\.words).max() ?? 0
    }
    ...
    var mostWordsInDay: Int {
        ...
        for record in self.summaryRecords {
            let day = calendar.startOfDay(for: record.timestamp)
            dayTotals[day, default: 0] += record.words
        }
    ...
    var transcriptionMilestones: [(target: Int, achieved: Bool, label: String)] {
        ...
        let total = self.totalEntryCount
        return milestones.map { (target: $0.0, achieved: total >= $0.0, label: $0.1) }
    }
