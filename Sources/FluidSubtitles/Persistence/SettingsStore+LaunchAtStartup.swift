...
extension SettingsStore {
        ...
        do {
            ...
            if self.launchAtStartupEnabled != enabled {
                let mismatchMessage = enabled
                    ? "macOS did not enable \(FluidProduct.displayName) in Login Items. Check System Settings > General > Login Items."
                    : "macOS still shows \(FluidProduct.displayName) in Login Items. Check System Settings > General > Login Items."
    ...
    private func launchAtStartupFailureMessage(for error: Error, enabling: Bool) -> String {
        ...
        {
            return "\(FluidProduct.displayName) could not \(action) launch at startup. This build may not be signed correctly for macOS Login Items."
        }

        if lowercasedDescription.contains("approval") ||
            lowercasedDescription.contains("authorize")
        {
            return "macOS needs approval before \(FluidProduct.displayName) can \(action) launch at startup. Check System Settings > General > Login Items."
        }

        return "\(FluidProduct.displayName) could not \(action) launch at startup. macOS reported: \(nsError.localizedDescription)"
    }
    ...
    private var compatibilityLoginItemName: String {
        Bundle.main.object(forInfoDictionaryKey: "CFBundleName") as? String ?? FluidProduct.displayName
    }
    ...
    private func runLaunchAtStartupAppleScript(_ source: String) throws {
        ...
        guard let script = NSAppleScript(source: source) else {
            throw NSError(
                domain: "fluidSubtitlesLaunchAtStartup",
                code: 1,
...
private enum LaunchAtStartupSystemState {
    ...
    var message: String {
        switch self {
        case .enabled:
            return "\(FluidProduct.displayName) reflects the actual macOS login item state."
        case .disabled:
            return "\(FluidProduct.displayName) reflects the actual macOS login item state. Unsigned or development builds may fail to enable this."
        case .requiresApproval:
            return "macOS requires approval for \(FluidProduct.displayName) in Login Items before launch at startup becomes active."
        }
    }
}
#endif
