...
task_test_dir=$(mktemp -d /tmp/fluidsubtitles-paste-cache-tests.XXXXXX)
xcrun swiftc -O \
    "$task_repo_dir/Sources/FluidSubtitles/Services/PasteKeyCodeCache.swift" \
...
xcrun swiftc -O \
    "$task_repo_dir/Sources/FluidSubtitles/Services/PasteKeyCodeResolver.swift" \
