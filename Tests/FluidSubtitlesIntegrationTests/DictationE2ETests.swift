...
final class DictationE2ETests: XCTestCase {
    ...
    func testDictionaryTransferDocument_encodesSimpleUserFormat() throws {
        let document = DictionaryTransferDocument(
            replacements: [
                DictionaryTransferReplacement(from: ["fluid subtitles", "connecting boys"], to: "fluidSubtitles"),
            ],
            customWords: ["fluidSubtitles", "GEMBA-E"]
        )
        ...
        XCTAssertEqual(firstReplacement["from"] as? [String], ["fluid subtitles", "connecting boys"])
        XCTAssertEqual(firstReplacement["to"] as? String, "fluidSubtitles")
        XCTAssertEqual(root["customWords"] as? [String], ["fluidSubtitles", "GEMBA-E"])
    ...
    func testDictionaryTransferImport_replaceMapsSimpleFormatToStores() throws {
        let document = DictionaryTransferDocument(
            ...
            customWords: [" fluidSubtitles ", "fluidsubtitles", " Barath "]
        )
        ...
        XCTAssertEqual(state.replacements.first?.triggers, ["fluid subtitles", "connecting boys"])
        XCTAssertEqual(state.replacements.first?.replacement, "fluidSubtitles")
        XCTAssertEqual(state.customWords.map(\.text), ["fluidSubtitles", "Barath"])
    ...
    func testDictionaryTransferImport_mergeDedupesAndMovesDuplicateTriggers() throws {
        let oldReplacement = SettingsStore.CustomDictionaryEntry(
            triggers: ["fluid subtitles", "old trigger"],
            replacement: "Old"
        )
        let existingReplacement = SettingsStore.CustomDictionaryEntry(
            triggers: ["connecting boys"],
            replacement: "fluidSubtitles"
        )
        ...
        let fluidVoiceEntry = try XCTUnwrap(state.replacements.first { $0.replacement == "fluidSubtitles" })
        ...
        XCTAssertEqual(Set(fluidVoiceEntry.triggers), Set(["fluid subtitles", "connecting boys"]))
    ...
    func testDictionaryTransferImport_acceptsAppStyleReplacementKeysAndSingleFromValue() throws {
        ...
        XCTAssertEqual(state.replacements.map(\.triggers), [["fluid subtitles"], ["gemba e"]])
        XCTAssertEqual(state.replacements.map(\.replacement), ["fluidSubtitles", "GEMBA-E"])
    }
    ...
    func testDictionaryTransferImport_acceptsLocalAPIReplacementItemsResponse() throws {
        ...
        XCTAssertEqual(state.replacements.first?.triggers, ["fluid subtitles"])
        XCTAssertEqual(state.replacements.first?.replacement, "fluidSubtitles")
    ...
    func testDictionaryTransferImportFeedsActualReplacementPath() throws {
        ...
        self.withRestoredDefaults(keys: [self.customDictionaryEntriesKey]) {
            ...
            XCTAssertEqual(
                ASRService.applyCustomDictionary("I use fluid subtitles daily."),
                "I use fluidSubtitles daily."
            )
        }
    }
...
extension DictationE2ETests {
    func testDictionaryTrainingNormalizesSamplesAndIgnoresIntendedText() {
        let triggers = CustomDictionaryTrainingMerge.normalizedTriggers(
            from: [" Fluid Subtitles. ", "fluidSubtitles", "fluid subtitles", " "],
            intendedReplacement: "fluidSubtitles"
        )

        XCTAssertEqual(triggers, ["fluid subtitles"])
    }
    ...
    func testDictionaryTrainingMergeDedupesAndMovesDuplicateTriggers() {
        ...
        let entries = CustomDictionaryTrainingMerge.mergedEntries(
            ...
            replacement: " fluidSubtitles ",
            triggers: ["Fluid Subtitles.", "connecting boys", "fluidSubtitles", ""]
        )

        let fluidVoiceEntry = entries.first { $0.replacement == "fluidSubtitles" }
        let oldEntry = entries.first { $0.replacement == "Old" }

        XCTAssertEqual(entries.count, 2)
        XCTAssertEqual(entries.map(\.replacement), ["fluidSubtitles", "Old"])
        XCTAssertEqual(Set(fluidVoiceEntry?.triggers ?? []), Set(["fluid subtitles", "connecting boys"]))
        XCTAssertEqual(oldEntry?.triggers, ["old trigger"])
    }
    ...
    func testDictionaryTrainingNewReplacementPrependsEntry() {
        ...
        XCTAssertEqual(entries.map(\.replacement), ["fluidSubtitles", "Existing"])
        XCTAssertEqual(entries.first?.triggers, ["fluid subtitles"])
    }
    ...
    func testManualDictionaryEntryParsesCommaSeparatedVariants() {
        XCTAssertEqual(
            CustomDictionaryManualEntry.normalizedDraftTriggers("fluid subtitles, connecting boys, fluid subtitles"),
            ["fluid subtitles", "connecting boys"]
        )
    }
    ...
    func testAutomaticDictionaryCorrectionRejectsNewWordAtDictationEnd() {
        let before = "fluidSubtitles works"
        let after = "fluidSubtitles works well"
        let insertedRange = NSRange(location: 0, length: (before as NSString).length)
        let change = AutomaticDictionaryCorrectionDetector.textChange(before: before, after: after)
    ...
    func testPronunciationProfileEditPolicyDiscardsProfileWhenMeaningChanges() {
        XCTAssertTrue(
            PronunciationProfileEditPolicy.shouldDiscardProfile(
                ...
                updatedReplacement: "fluidSubtitles"
            )
        )
        XCTAssertFalse(
            PronunciationProfileEditPolicy.shouldDiscardProfile(
    ...
    func testProgressiveDownloaderRetainsFileByMovingIt() throws {
        let source = FileManager.default.temporaryDirectory
            .appendingPathComponent("fluidSubtitlesDownloadSource-\(UUID().uuidString)")
    ...
    func testDictionaryTransferImport_acceptsParakeetVocabularyTermsFile() throws {
        ...
        XCTAssertEqual(state.customWords.map(\.text), ["fluidSubtitles", "GEMBA-E"])
    ...
    func testDictationEndToEnd_whisperTiny_transcribesFixture() async throws {
        ...
        XCTAssertTrue(
            normalized.contains("voice") || normalized.contains("boys"),
    ...
    func testPrivateAIProviderLocalRuntimeOnlyHandlesPrivateModels() {
        self.withRestoredDefaults(keys: [self.privateAILocalModelPathKey]) {
            let tempURL = FileManager.default.temporaryDirectory
                .appendingPathComponent("fluidSubtitles-PrivateAI-\(UUID().uuidString).gguf")
    ...
    func testRollbackBackupsPreferFilenameTimestampOverModificationDate() {
        let firstBackupWithNewestModificationDate = URL(
            fileURLWithPath: "/tmp/fluidSubtitles-1.5.11-beta.1-100.app"
        )
        let secondBackup = URL(
            fileURLWithPath: "/tmp/fluidSubtitles-1.5.11-beta.2-150.app"
        )
        let thirdBackup = URL(
            fileURLWithPath: "/tmp/fluidSubtitles-1.5.11-beta.3-rollback-200.app"
        )
        let fourthBackupWithOldestModificationDate = URL(
            fileURLWithPath: "/tmp/fluidSubtitles-1.5.11-beta.4-rollback-300.app"
        )
    ...
    func testValidateDownloadedFile_rejectsHTMLBodyAndAcceptsJSON() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("fluidSubtitles-ValidateTest-\(UUID().uuidString)", isDirectory: true)
    ...
    func testCachedFileIsMarkup_detectsCachedCorruptHTMLAndAcceptsModelData() throws {
        ...
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("fluidSubtitles-CachedMarkupTest-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testCachedPayloadContainsMarkup_detectsCorruptFileInPresentArtifactTree() throws {
        ...
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("fluidSubtitles-CachedPayloadTest-\(UUID().uuidString)", isDirectory: true)
    ...
    private static func modelDirectoryForRun() -> URL {
        ...
        let base = FileManager.default.temporaryDirectory
            .appendingPathComponent("fluidSubtitlesTests", isDirectory: true)
